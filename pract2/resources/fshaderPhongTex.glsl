...
void main()
{
    ...
    for(int i = 0; i < numLights.numL; i++){ // per a cada llum de la escena
        ...
        vec4 diffuse4 = 0.25*vec4(diffuse, 1.0)+ 0.75 * texture2D(texture, coordTex);
        diffuse = vec3(diffuse4.x, diffuse4.y, diffuse4.z);
        c += (ambient + diffuse + specular)*atenuacio;

    }
    color = vec4(c, 1.0);//*0.25 + 0.75 * texture2D(texture, coordTex);
