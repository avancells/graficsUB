...
void GLWidget::initShadersGPU(){    
    ...
    initShader("://resources/vshaderPhongTex.glsl", "://resources/fshaderPhongTex.glsl");
...
void GLWidget::newObj(QString fichero){
    ...
    qDebug() << fichero;
    Object * obj = new Object(1000000, fichero);
    obj->toGPU(program);
    scene->addObject(obj);
...
void GLWidget::newScene() {
    ...
    vec4 lookfrom(-5.74, 5.26, 5.99, 1.0);
    vec4 lookat(0.33, 0.61, 0.45, 1.0);
    vec4 vup(0.00, 0.77, -0.64, 1.0);

    scene->camera->actualitzaCamera(scene->capsaMinima);

    scene->camera->setFrustum(119.60, 0.01, 100.00);
